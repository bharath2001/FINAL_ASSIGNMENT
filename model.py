...
#ARTIFICIAL NEURAL NETWORK :  Accuracy - 99.99% 
...
x = dataset.iloc[:, [2,3,4,5,6,8,12,14,16,18,20,22]]    #splitting independent features and
...
df=pd.get_dummies(dataset.Location)     #dummy encoding the independent features
data_frame=pd.concat([df,x],axis=1)       #concatenating location with the rest features
...
#here i have considered 13 independent features
...
classifier.add(Dense(output_dim = 15, init = 'uniform', activation = 'relu', input_dim =60))
...
classifier.fit(x_train, y_train, batch_size = 1500, nb_epoch = 150)
...
#NOTE: here i always get the same accuracy beacuse each time the dataset is splitted to x and y, it splits in the same order as i have set "random_state=0"